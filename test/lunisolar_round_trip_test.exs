defmodule Calendrical.Chinese.RoundTrip.Test do
  use ExUnit.Case
  ...
  property "Calendar Conversion Round Trip" do
    check all(iso_days <- Chinese.DateGenerator.generate_iso_days(), max_runs: @max_runs) do
      chinese_date = Calendrical.date_from_iso_days(iso_days, Calendrical.Chinese)
      gregorian_date = Date.convert!(chinese_date, Calendrical.Gregorian)
      gregorian_iso_days = Calendrical.date_to_iso_days(gregorian_date)
      chinese_iso_days = Calendrical.date_to_iso_days(chinese_date)
  ...
  test "Cycle and Year" do
    ...
    for year <- from..to do
      cycle_and_year = Calendrical.Chinese.cycle_and_year(year)
      assert Calendrical.Chinese.elapsed_years(cycle_and_year) == year
    end
  end
  ...
  test "Round trip" do
    ...
    to = Calendrical.date_to_iso_days(~D[2025-12-31])

    for iso_days <- from..to do
      chinese_date = Calendrical.Chinese.chinese_date_from_iso_days(iso_days)
      assert Calendrical.Chinese.chinese_date_to_iso_days(chinese_date) == iso_days
    end
  end
  ...
  test "Calendar Conversion" do
    ...
    for date <- dates do
      iso_days = Calendrical.date_to_iso_days(date)
      chinese_date = Calendrical.date_from_iso_days(iso_days, Calendrical.Chinese)
      chinese_iso_days = Calendrical.date_to_iso_days(chinese_date)
