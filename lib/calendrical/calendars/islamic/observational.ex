defmodule Calendrical.Islamic.Observational do
  ...
  * A `t:Geo.PointZ.t/0` for Cairo, Egypt (Reingold's canonical
  ...
  @doc """
  Returns the observational Hijri date in effect at a given instant, under a
  chosen day-start convention.

  The Islamic day begins at sunset, so after sunset at the observation point
  (Cairo) the date is already the following day. This maps an absolute instant
  to the Hijri date, choosing the boundary with the `:day_start` option, always
  evaluated **at Cairo** (the calendar's reference location).
  ...
  * `datetime` is a `t:DateTime.t/0` — an absolute instant. Its own time zone
    fixes the instant; the day boundary is always taken at Cairo.

  ### Options

  * `:day_start` selects the moment the day begins:

    * `:midnight` (the default) — 00:00 Cairo standard time, the ordinary
      civil-day mapping.

    * `:evening` — 18:00 Cairo standard time, the fixed-clock proxy for sunset.

    * `:sunset` — true sunset (Maghrib) at Cairo, computed via `Astro`.

  ### Returns

  * `{:ok, date}` — an observational Hijri `t:Date.t/0`.
  ...
  * `{:error, reason}` if the day-start option is invalid, the date lies outside
  ...
  ### Examples
      ...
      iex> {:ok, date} = Calendrical.Islamic.Observational.date_at(~U[2025-03-01 06:00:00Z])
      iex> date.calendar
  ...
  def date_at(%DateTime{} = datetime, options \\ []) do
    Calendrical.DayStart.date_at(datetime, __MODULE__, location(), 2 * 60 * 60, options)
  end
  ...
  * A two-tuple `{year, week_in_year}`.
  ...
  ### Examples
      ...
      iex> Calendrical.Islamic.Observational.week_of_year(1447, 1, 1)
      {1447, 1}

      iex> Calendrical.Islamic.Observational.week_of_year(1447, 9, 1)
      {1447, 35}
  ...
  * A two-tuple `{weeks_in_year, days_in_last_week}` where the
  ...
  ### Examples
      ...
      iex> Calendrical.Islamic.Observational.weeks_in_year(1447)
      {52, 2}
