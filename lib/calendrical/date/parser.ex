defmodule Calendrical.Date.Parser do
  ...
  # Hebrew "Adar II"). It becomes the month of the date only once the
  ...
  defp parse_or_wrap(string, options, reason_tag) do
    ...
    case parse(string, options) do
      ...
      # unknown calendar, is reported as it stands.
      {:error, _other} = error ->
        error
    end
  end
  ...
  defp match_pattern(input, pattern, ctx, as) do
    ...
    with %Regex{} <- regex,
         ...
         {:ok, fields} <-
           extract_fields(caps, year_fallback, era_index, ctx.calendar_module) do
  ...
  # full Date.
  defp extract_fields(caps, reference_year, era_index, calendar_module) do
  ...
  # A month name gives CLDR's number for the month. It is the month of
  # the date only in a calendar that numbers its months that way; the
  # month is the one of `year` whose `month_of_year/3` (the key the
  # calendar's month names are found by) is that number. So a Hebrew
  # month after Adar I sits one place earlier in an ordinary year, and a
  # Chinese month one place later after a leap month. A plain name of a
  # month that `year` has only as a leap variant (Adar, in a Hebrew leap
  # year) names that variant. A name the year has no month for gives
  # `nil`. Without a year, the month is CLDR's number.
  ...
  # A week-based calendar's dates carry a week, not a month, so its
  # month names are not positions in the year.
