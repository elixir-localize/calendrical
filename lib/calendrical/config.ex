defmodule Calendrical.Config do
  ...
  defstruct calendar: nil,
            ...
            # The functions `Calendrical.monday()`
            # etc can be used
            day_of_week: 1,
  ...
  def extract_options(options) do
    ...
    %__MODULE__{
      ...
      min_days_in_first_week: min_days_for_locale(options),
      day_of_week: first_day_for_locale(options),
  ...
  def min_days_for_locale(options) do
    default =
      if locale = Keyword.get(options, :locale) do
        Calendrical.min_days_for_locale(locale)
      else
        1
      end
  ...
  def first_day_for_locale(options) do
    default =
      if locale = Keyword.get(options, :locale) do
        Calendrical.first_day_for_locale(locale)
      else
        1
      end
