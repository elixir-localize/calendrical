defmodule Calendrical.Duration do
  @moduledoc """
  ...
  The [ex_localize_units](https://hex.pm/packages/ex_localize_units) package can
  ...
  if Code.ensure_loaded?(Localize.Unit) do
    ...
    * `duration` is a duration of type `t:Calendrical.Duration.t/0` returned
    ...
    * `locale` is any valid locale name returned by `Localize.known_locale_names/0`
      or a `Localize.LanguageTag` struct returned by `Localize.Locale.new!/2`.
      The default is `Localize.get_locale/0`.

    * `:list_options` is a list of options passed to `Localize.List.to_string/3` to
      control the final list output.

    Any other options are passed to `Localize.Number.to_string/3` and
    `Localize.Unit.to_string/3` during the formatting process.
    ...
    * Any duration parts that are `0` are not output.
    ...
    ## Example
        ...
        iex> {:ok, duration} = Calendrical.Duration.new(~D[2019-01-01], ~D[2019-12-31])
        iex> Calendrical.Duration.to_string(duration)
        {:ok, "11 months and 30 days"}

    """
    def to_string(%__MODULE__{} = duration, options \\ []) do
      {except, options} = Keyword.pop(options, :except, [])
      {list_options, options} = Keyword.pop(options, :list_options, [])
      ...
      unit_strings =
        for key <- @keys,
          ...
          Localize.Unit.to_string!(Localize.Unit.new!(value, Atom.to_string(key)), options)
        end

      Localize.List.to_string(unit_strings, list_options)
    end
    ...
    * `options` is a Keyword list of options.
    ...
    See `Calendrical.Duration.to_string/2`.
    ...
    * raises an exception.
    ...
    [Calendrical, Localize.Number, Localize.Unit, Localize.List]`
  ...
  * `{:error, {exception, reason}}` tuple
  ...
  ## Example
      ...
      iex> Calendrical.Duration.new(~D[2019-01-01], ~D[2019-12-31])
      {:ok,
  ...
  def new(unquote(Calendrical.datetime()) = from, unquote(Calendrical.datetime()) = to) do
  ...
  def new(unquote(Calendrical.date()) = from, unquote(Calendrical.date()) = to) do
  ...
  def new(unquote(Calendrical.time()) = from, unquote(Calendrical.time()) = to) do
    with {:ok, from} <- cast_date_time(from),
         {:ok, to} <- cast_date_time(to) do
      time_diff = time_duration(from, to)
      {seconds, microseconds} = Localize.Utils.Math.div_mod(time_diff, 1_000_000)
      {minutes, seconds} = Localize.Utils.Math.div_mod(seconds, 60)
      {hours, minutes} = Localize.Utils.Math.div_mod(minutes, 60)
  ...
  * `CalendarInterval` is defined by the most wonderful
  ...
  ## Example
      ...
      iex> Calendrical.Duration.new(Date.range(~D[2019-01-01], ~D[2019-12-31]))
      {:ok,
       %Calendrical.Duration{
         year: 0,
         month: 11,
  ...
  * a `t:Calendrical.Duration.t/0`

  ### Example

      iex> Calendrical.Duration.new_from_seconds(36092.1)
      %Calendrical.Duration{
        year: 0,
        month: 0,
        day: 0,
  ...
  defp microseconds_from_fraction(number) do
    fraction = Localize.Utils.Digits.fraction_as_integer(number)
  ...
  defp cast_date_time(unquote(Calendrical.datetime()) = datetime) do
    _ = calendar
    {:ok, datetime}
  end

  defp cast_date_time(unquote(Calendrical.naivedatetime()) = naivedatetime) do
    _ = calendar
    DateTime.from_naive(naivedatetime, "Etc/UTC")
  end

  defp cast_date_time(unquote(Calendrical.date()) = date) do
    {:ok, dt} = NaiveDateTime.new(date.year, date.month, date.day, 0, 0, 0, {0, 6}, calendar)
    DateTime.from_naive(dt, "Etc/UTC")
  end

  defp cast_date_time(unquote(Calendrical.time()) = time) do
    {:ok, dt} =
      NaiveDateTime.new(
        1,
        1,
  ...
  defp confirm_same_time_zone(from, to) do
    {:error,
     {Calendrical.IncompatibleTimeZone,
  ...
  * A `t:Calendrical.Duration.t/0` struct or
  ...
  * `CalendarInterval` is defined by the most wonderful
  ...
  defp time_duration(unquote(Calendrical.time()) = from, unquote(Calendrical.time()) = to) do
  ...
  defp back_one_day(%{day: 0} = date_diff, :day, from) do
    months_in_year = Calendrical.months_in_year(from)
    previous_month = Localize.Utils.Math.amod(from.month - 1, months_in_year)
  ...
  defp merge(duration, microseconds) do
    {seconds, microseconds} = Localize.Utils.Math.div_mod(microseconds, @microseconds_in_second)
