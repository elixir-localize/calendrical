defmodule Calendrical.Sigils do
  @moduledoc """
  ...
  * `~d[yyyy-mm-dd]` which produces a date in the `Calendrical.Gregorian` calendar
  * `~d[yyyy-Wmm-dd]` which produces a date in the `Calendrical.ISOWeek` calendar
  ...
  * `~d[yyyy-mm-dd C.E Julian]` which produces a date in the Calendrical.Julian calendar
  * `~d[yyyy-mm-dd B.C.E Julian]` which produces a date in the Calendrical.Julian calendar

  ## Examples

      iex> import Calendrical.Sigils
  ...
  def do_sigil_d(<<year::bytes-4, "-", month::bytes-2, "-", day::bytes-2>>, _) do
    to_date(year, month, day, Calendrical.Gregorian)
  end

  def do_sigil_d(<<year::bytes-4, "-", month::bytes-2, "-", day::bytes-2, " C.E. Julian">>, _) do
    to_date(year, month, day, Calendrical.Julian)
  end
  ...
  def do_sigil_d(<<year::bytes-4, "-W", month::bytes-2, "-", day::bytes-2>>, _) do
    to_date(year, month, day, Calendrical.ISOWeek)
  end
  ...
  defp inbuilt_calendar(calendar) do
    calendar = Module.concat(Calendrical, calendar)
    get_calendar(calendar)
  end

  defp fiscal_calendar(calendar) do
    calendar = Module.concat(Calendrical.FiscalYear, calendar)
    get_calendar(calendar)
  end
  ...
  defp get_calendar(calendar) do
    if Code.ensure_compiled(calendar) == {:module, calendar} and
         function_exported?(calendar, :cldr_calendar_type, 0) do
      {:ok, calendar}
    else
      nil
    end
